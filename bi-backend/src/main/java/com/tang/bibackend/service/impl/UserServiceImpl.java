...
import com.tang.bibackend.mapper.UserMapper;
import com.tang.bibackend.model.dto.user.UserQueryRequest;
import com.tang.bibackend.model.entity.User;
import com.tang.bibackend.model.enums.UserRoleEnum;
import com.tang.bibackend.model.vo.LoginUserVO;
import com.tang.bibackend.model.vo.UserVO;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public LoginUserVO userLogin(String userAccount, String userPassword, HttpServletRequest request) {
        ...
        String encryptPassword = DigestUtils.md5DigestAsHex((SALT + userPassword).getBytes());
        // 查询用户是否存在
        User loginUser = (User) redisTemplate.opsForValue().get("loginUser_" + userAccount);
        if (loginUser == null) {
            // redis中不存在用户信息
            QueryWrapper<User> queryWrapper = new QueryWrapper<>();
            queryWrapper.eq("userAccount", userAccount);
            queryWrapper.eq("userPassword", encryptPassword);
            loginUser = this.baseMapper.selectOne(queryWrapper);
            //存入redis
            redisTemplate.opsForValue().set("loginUser_" + userAccount, loginUser);
            redisTemplate.expire("loginUser_" + userAccount, 3600, TimeUnit.SECONDS);
        }
        // 用户不存在
        if (loginUser == null) {
    ...
    public User getLoginUser(HttpServletRequest request) {
        ...
        currentUser = (User) redisTemplate.opsForValue().get("loginUser_" + userAccount);
        if (currentUser == null) {
            throw new BusinessException(ErrorCode.NOT_LOGIN_ERROR);
        }
        return currentUser;
